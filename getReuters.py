...
opts, args = getopt.getopt(sys.argv[1:], 'f:c:m:', ['lsa', 'idf'])
for o, a in opts:
    if o == "-f":
        max_features = int(a)
    elif o == '-c':
...
if use_lsa:
    lsa = TruncatedSVD(components, random_state=1)
...
DFTrain.join(DFTrainLabels, lsuffix="_t").to_csv('train.csv', header=True, index=False)
...
DFTest.join(DFTestLabels, lsuffix="_t").to_csv('test.csv', header=True, index=False)
